...
@if($val == "epargne")
    ...
    ?>
                ...
                <div>
                    ...
                    <select onchange="ecran($(this).val(),'idlistecompte','employe.contenu.versementfinepargne')" class="form-control show-tick"  name="client" >
                        <option></option>
...
@if($val == "courant")
    ...
    <div>
        <label for="clients">Nom du Client</label>
        <select onchange="ecran($(this).val(),'idlistecompte','employe.contenu.versementfincourant')" class="form-control show-tick"  name="client" >
            <option></option>
            @foreach($client as $client)
                <option value="{{ $client->id }}" >{{$client->nom}}  {{$client->prenom}} </option>
            @endforeach
